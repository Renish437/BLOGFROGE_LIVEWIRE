  @props(['post'])
  <article
  {{ $attributes->merge(['class' => '[&:not(:last-child)]:border-b border-gray-100 pb-10 dark:text-gray-100']) }}
   >
                            <div class="article-body grid grid-cols-12 gap-3 mt-5 items-start">
                                <div class="article-thumbnail col-span-4 flex items-center">
                                    <a class="aspect-w-3 aspect-h-3" wire:navigate href="{{ route('posts.show', $post) }}" >

                                        <img class="w-full mx-auto rounded-xl"
                                            src="{{ $post->getThumbnailImage() }}"
                                            alt="thumbnail">
    

                                    </a>
                                </div>
                                <div class="col-span-8">
                                    <div class="article-meta flex py-1 text-sm items-center">
                                        <x-posts.author :author="$post->author" />
                                        <span class="text-gray-500 dark:text-gray-100 text-xs">.{{ $post->published_at->diffForHumans() }}</span>
                                    </div>
                                    <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                                        <a wire:navigate  href="{{ route('posts.show', $post) }}" >
                                            {{ $post->title }}
                                        </a>
                                    </h2>
                        
                                    <p class="mt-2 text-base text-gray-700 dark:text-gray-100 font-light">
                                        {!! Str::limit($post->body, 200) !!}
                                    </p>
                                    <div class="article-actions-bar mt-6 flex items-center justify-between">
                                        <div class="flex items-center space-x-2">
                                            @foreach ($post->categories as $category)
                                                  <x-posts.category-badge :category="$category" />
                                                
                                            @endforeach
                                        <span class="text-gray-500 dark:text-gray-100 text-sm">{{ $post->getReadingTime() }} min read</span>
                                        </div>
                                        <div class="flex  space-x-4">
                                           
                                        </div>
                                        <div>
                                          <livewire:like-button :post="$post" :key="$post->id.now()"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </article>